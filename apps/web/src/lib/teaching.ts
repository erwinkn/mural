// Port of apps/ios/Core/TeachingPolicy.swift — prompts shared verbatim with
// the native apps so the conversation behaves the same way.

import type { LanguageModule } from './languages'
import type { ConversationTheme } from './themes'
import type { LearnerState } from './learning'
import type { Passage, SessionRecord } from './models'
import { passageText, sessionPassages } from './models'
...
export function voiceInstructions(
  ...
  const focus = language.teachingFocus[Math.min(5, Math.max(0, learner.challenge))]
...
export function assessmentInstructions(language: LanguageModule): string {
...
export function greetingInstructions(language: LanguageModule): string {
...
export function helpInstructions(language: LanguageModule): string {
...
export function redirectInstructions(language: LanguageModule): string {
...
/// NaturalLanguage reports Chinese script IDs (zh-Hans / zh-Hant); these
/// describe the transcript's script, not a different spoken language.
export function shouldRedirectSpeech(
  language: LanguageModule,
...
export function themeInstructions(theme: ConversationTheme | null, language: LanguageModule): string {
...
export function translationInstructions(language: LanguageModule, meaningLanguage: string): string {
...
export function delegationInstructions(language: LanguageModule): string {
...
export function typedReplyInstructions(language: LanguageModule): string {
...
export function lookupInstructions(language: LanguageModule, meaningLanguage: string): string {
...
export function currentTopicInstructions(language: LanguageModule): string {
...
export function contextFor(session: SessionRecord, passage?: Passage): string {
  const rows = sessionPassages(session)
    .slice(-10)
    .map((p) => `${p.speaker.toUpperCase()} [${p.fragments.map((f) => f.id).join(',')}]: ${passageText(p)}`)
    .join('\n')
  if (!passage) return `TARGET LANGUAGE: ${session.languageID}\n${rows}`
  const fragments = passage.fragments
